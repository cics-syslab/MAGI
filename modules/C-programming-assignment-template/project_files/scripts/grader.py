...
class TestCase:
    ...
    def __str__(self):
        return "TestCase: {" + "name: " + self.name + ", status: " + self.status + ", time: " + self.time + ", classname: " + self.classname + ", failures: " + str(
            self.failures) + ", msg: " + self.msg + "}"
...
def getTestSuite(xmldoc) -> Test:
    ...
    return Test(int(tests), int(failures), int(disabled), int(errors), timestamp, time, name,
                itemlist.getElementsByTagName('testsuite'))
...
def generateJSON(test_result: Test, points: int, verbose: bool) -> str:
    ...
    for result in test_result:
        ...
        if result.status == "error":
            results_list.append(json.dumps(generateFailureJSON(
                result.name + " crashed the test case and was unable to succesfully complete. You may have a Segmentation Fault in your code. Please re-examine your code and re-upload.",
                points, result.name)))
            continue
    ...
    return ",".join(
        results_list) + ","  # Put commas between all the test results and put one at the end for the next one.
