...
def generate_code(template_file, output_file, variables):
    ...
    env = Environment(loader=FileSystemLoader("."))  # Assuming the templates is in the same directory
    # Load the templates
    with open(template_file, "r") as template_file:
        template_content = template_file.read()
    converted_template = convert_template(template_content)

    # Rendering the transformed templates file using Jinja
...
if __name__ == "__main__":
    ...
    # Check if templates file is provided as a command-line argument
    if len(sys.argv) < 2:
        print("Usage: python generate_source.py <template_file>")
        sys.exit(1)
    # Get the templates file from the command-line argument
